...
#if BITS_PER_LONG == 64
			mask = 0xffffffff << (offset - long_offset);
#elif BITS_PER_LONG == 32
			mask = 0xffff << (offset - long_offset);
#endif
			tmp = b[i] & mask;
		}
		else {
...
static inline int resch_fps(unsigned long *b, int offset, int len)
{
	...
	for (i = len - 1; i >= 0; i--) {
		...
		if (long_offset + BITS_PER_LONG > offset) {
#if BITS_PER_LONG == 64
			mask = 0xffffffff >> ((BITS_PER_LONG - 1) - (offset - long_offset));
#elif BITS_PER_LONG == 32
			mask = 0xffff >> ((BITS_PER_LONG - 1) - (offset - long_offset));
#endif
			tmp = b[i] & mask;
		}
		else {
