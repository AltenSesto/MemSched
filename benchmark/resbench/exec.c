 ...
 */
/*
 * exec.c:	execute tasks in the given taskset and measure success ratio.
 */
...
int schedule(FILE *fp, int m, int loop_1ms, int simtime)
{
	int i, j, ret, status;
	pid_t pid;
	...
	for (i = 0; i < nr_tasks; i++) {
		...
		if (pid == 0) { /* the child process. */
			...
			exec_task(&tasks[i], loop_1ms, simtime);
		}
	}
