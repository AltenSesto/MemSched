...
//	item *curr2 , * head2 = NULL;
	...
	exec = 2*atoi(argv[3])*MSEC_UNIT;//the task executes 2/3 of wcet in a period
...
//		}
		...
		if ((elapsed_time = difftime(time(NULL), start)) >= runtime){
			...
			printf("%s Task missed counts =  %d out of %d number of nodes %ld memory size %ld  \n", task_name, deadlinemiss_count, count, exec, sizeof(item)*exec);
			rt_exit();
			exit(EXIT_SUCCESS);		
		}
		++count;
